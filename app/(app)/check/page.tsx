...
export default async function CheckLandingPage() {
  ...
  const isOperator = hasRole(roles, "plant_operator");

  // Daily checks are for drivers and plant operators only.
  if (!isDriver && !isOperator) redirect("/dashboard");
  ...
  return (
    <>
      ...
      <p className="field-hint" style={{ marginTop: -8, marginBottom: 16 }}>
        Pick what you&apos;re using today.
      </p>

      {v.length === 0 && p.length === 0 && (
        <div className="card">
