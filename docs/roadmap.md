...
- **System-wide edit/delete pass** — partly done 2026-09-05: archive-style delete
  (admin / transport manager, **detail page only**, confirm) added to individual
  **faults** (`voidFault`), **inspections** (`voidInspection`) and **vehicle
  inspections** (`voidVehicleInspection` — all three tables already had `voided`);
  the **service** delete is now manager-only. Still to do: an Edit on services, and
  a sweep of the remaining list screens for missing Edit links.
